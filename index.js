...
// ************************************************************
// ************************************************************
const MINOXDOMAIN = "https://app.minox.nl";
let CLIENTDOMAIN = "";
const CLIENTID = "";
const CLIENTSECRET = "";
const REQUESTEDSCOPES =
  "administration:read supplier:read customer:read ledger_account:read journal:read vat:read transaction:read transaction:write";
...
if (!CLIENTSECRET || !CLIENTID) {
  console.log(
    "The application cannot start until CLIENTSECRET and CLIENTID are set."
  );
  process.exit(-1);
}
...
app.get("/", async (request, response) => {
  let html =
    "<!DOCTYPE html><html><head><title>Minox client demo</title></head><body><h1>Welkom bij Minox client demo</h1>";
...
app.get("/dashboard", authenticated, async (request, response) => {
  try {
    ...
    for (const tenant of request.user.linked_tenants) {
      let tenantAdministrations = (await getAdministrations(
        request.user.token,
        tenant.tenant_id
      )).collection;
      tenantAdministrations = tenantAdministrations.map(administration => ({
        ...tenant,
        ...administration
      }));
    ...
    let html =
      "<!DOCTYPE html><html><head><title>Minox client demo</title></head><body><h1>Welkom bij Minox client demo</h1><h2>We kunnen met de onderstaande tenants/administraties verbinden</h2><table><thead><tr><th>TenantID</th><th>AdministrationID</th><th>Company name</th><th>Number of periods</th><th>Chamber of commerce</th></tr></thead><tbody>";
    html += administrations
      .map(
        administration =>
          `<tr><td>${administration.tenant_id}</td><td>${
            administration.id
          }</td><td>${administration.name || ""}</td><td>${
            administration.number_of_periods
          }</td><td>${administration.chamber_of_commerce || ""}</td></tr>`
      )
      .join("");
